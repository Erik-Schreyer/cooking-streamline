...
void CookingMenuBar::openCookbookSlot()
{
    ...
    if (accepted && !cookBook.isEmpty()){
      ...
      QCookbook * openenedBook = new QCookbook(mainWindow_,this, cookBook.toStdString(),directoryPath+"/cookbooks/"+cookBook.toStdString());
...
void CookingMenuBar::openExtCookbookSlot()
{
    ...
    if (ok && !entries.isEmpty()){
      ...
      QCookbook * openenedBook = new QCookbook(mainWindow_,this,entries[0].toStdString(),entries[1].toStdString());
...
void CookingMenuBar::createCookbookActions()
{
    ...
    this->closeCookbook_ = new QAction("&Close Cookbook", this);
}
...
void CookingMenuBar::createRecepyActions()
{
    ...
    this->closeRecepy_ = new QAction("&Close Recepy", this);
...
void CookingMenuBar::addCookbookMenu(const char* name, QCookbook * openedBook){
    QMenu * cookbookMenu = new QMenu(tr(name));
    QAction * cookbookMenuAction = mainWindow_->menuBar()->addMenu(cookbookMenu);

    openedBook->thisMenuAction_=cookbookMenuAction;

    cookbookMenu->addAction(newRecepy_);
    cookbookMenu->addAction(loadRecepy_);
    cookbookMenu->addAction(removeRecepy_);
    cookbookMenu->addAction(closeCookbook_);
...
//    cookbookMenu->addAction(listRecepies_);
    ...
    connect(closeCookbook_, &QAction::triggered, openedBook, &QCookbook::closeCookbookSlot);

}

void CookingMenuBar::addRecepyMenu(const char *name, QRecepy *openedRecepy){
    QMenu * recepyMenu = new QMenu(tr(name));
    QAction *recepyMenuAction = mainWindow_->menuBar()->addMenu(recepyMenu);

    openedRecepy->thisMenuAction_=recepyMenuAction;
    ...
    recepyMenu->addAction(closeRecepy_);
    ...
    connect(closeRecepy_, &QAction::triggered, openedRecepy, &QRecepy::closeRecepySlot);
